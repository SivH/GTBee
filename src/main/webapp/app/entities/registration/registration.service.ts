...
export class RegistrationService {
    ...
    protected convertDateFromClient(registration: IRegistration): IRegistration {
        const copy: IRegistration = Object.assign({}, registration, {
            registrationDate:
                registration.registrationDate != null && registration.registrationDate.isValid()
                    ? registration.registrationDate.toJSON()
                    : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((registration: IRegistration) => {
                registration.registrationDate = registration.registrationDate != null ? moment(registration.registrationDate) : null;
            });
        }
        return res;
    }
