...
export class RegistrationUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.gTBactivityService
            .query()
            .pipe(
                filter((mayBeOk: HttpResponse<IGTBactivity[]>) => mayBeOk.ok),
                map((response: HttpResponse<IGTBactivity[]>) => response.body)
            )
            .subscribe((res: IGTBactivity[]) => (this.gtbactivities = res), (res: HttpErrorResponse) => this.onError(res.message));
    }
